...
# 生成的静态文件strip后会小很多
strip ./target/$target/release/$static_libname

arch2=aarch64
target2=${arch2}-apple-ios-sim
cargo build --target $target2 --release
# 生成的静态文件strip后会小很多
strip ./target/$target2/release/$static_libname

arch3=x86_64
target3=${arch3}-apple-ios
cargo build --target $target3 --release
# 生成的静态文件strip后会小很多
strip ./target/$target3/release/$static_libname
...
lipo ./target/$target2/release/$static_libname ./target/$target3/release/$static_libname -create -output $universal_path
...
cp ./target/$target/release/$static_libname $framework_path/ios-arm64/
